...
function matches(table: Table, term: string, pipe: PipeTransform) {
    return table.order_no.toLowerCase().includes(term.toLowerCase());
}
...
export class AccountListService {
    ...
    private _search(): Observable<SearchResult> {
        const { sortColumn, sortDirection, pageSize, page, searchTerm, status } = this._state;

        // 1. sort
        let tables = sort(OrdersData, sortColumn, sortDirection);
